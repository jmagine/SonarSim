                                        ...
                                        _receiverSocket(_ioService, ip::udp::endpoint(ip::udp::v4(), RECEIVER_BASE_PORT+_serverID))
{
    ...
    logger::setLogFilter(logger::severityLevel::info);
#else
    logger::setLogFilter(logger::severityLevel::trace);
#endif
#endif
...
dsm::Server::~Server() {
    ...
    LOG(_logger, logger::severityLevel::teardown) << "DESTROYED SERVER";
}
...
void dsm::Server::start() {
    ...
    LOG(_logger, logger::severityLevel::startup) << "MAIN LOOP START";
    ...
    while (_isRunning.load()) {
        ...
        switch(_message.options) {
            case CREATE_LOCAL:
                LOG(_logger, logger::severityLevel::info) << "LOCAL: " << (int)_message.clientID << " {" << _message.name << ", " << _message.footer.size << "}";
                createLocalBuffer(_message.name, _message.footer.size, _message.clientID, false);
                break;
            case CREATE_LOCALONLY:
                LOG(_logger, logger::severityLevel::info) << "LOCALONLY: " << (int)_message.clientID << " {" << _message.name << ", " << _message.footer.size << "}";
                createLocalBuffer(_message.name, _message.footer.size, _message.clientID, true);
                break;
            case FETCH_REMOTE:
                LOG(_logger, logger::severityLevel::info) << "REMOTE: " << (int)_message.clientID << " {" << _message.name << ", " << ip::address_v4(_message.footer.ipaddr) << ", " << (int)_message.serverID << "}";
                fetchRemoteBuffer(_message.name, _message.footer.ipaddr, _message.clientID, _message.serverID);
                break;
            case DISCONNECT_LOCAL:
                LOG(_logger, logger::severityLevel::info) << "REMOVE LOCAL LISTENER: " << (int)_message.clientID << " " << _message.name;
                disconnectLocal(_message.name, _message.clientID);
                break;
            case DISCONNECT_REMOTE:
                LOG(_logger, logger::severityLevel::info) << "REMOVE REMOTE LISTENER: " << (int)_message.clientID << " " << _message.name;
                disconnectRemote(_message.name, _message.footer.ipaddr, _message.clientID, _message.serverID);
                break;
            case DISCONNECT_CLIENT:
                LOG(_logger, logger::severityLevel::info) << "CLIENT DISCONNECTED: " << (int)_message.clientID;
                disconnectClient(_message.clientID);
                break;
            case CONNECT_CLIENT:
                LOG(_logger, logger::severityLevel::info) << "CLIENT CONNECTED: " << (int)_message.clientID;
                disconnectClient(_message.clientID);    //the reset and disconnect behavior is the same
                break;
            case CONNECT_CLIENT_NORESET:
                LOG(_logger, logger::severityLevel::info) << "CLIENT CONNECTED WITHOUT RESET: " << (int)_message.clientID;
                break;
            default:
                LOG(_logger, logger::severityLevel::error) << "UNKNOWN COMMAND";
                break;
        }
    }
    LOG(_logger, logger::severityLevel::teardown) << "MAIN LOOP END";
}

void dsm::Server::stop() {
    LOG(_logger, logger::severityLevel::teardown) << "SERVER STOPPING";
    _isRunning = false;
...
void dsm::Server::createLocalBuffer(dsm::LocalBufferKey key, uint16_t size, uint8_t clientID, bool localOnly) {
    LOG(_logger, logger::severityLevel::trace) << "CREATING LOCAL BUFFER: " << key;
    ...
    if (_localBufferMap->find(key) != _localBufferMap->end()) {
        ...
        if (!localOnly && _multicastPortOffsets[clientID] >= MAX_BUFFERS_PER_CLIENT) {
            LOG(_logger, logger::severityLevel::error) << "CLIENT " << (int)clientID << " HAS TOO MANY LOCAL BUFFERS";
            return;
        }
    }
...
void dsm::Server::createRemoteBuffer(dsm::RemoteBufferKey key, uint16_t size) {
    LOG(_logger, logger::severityLevel::trace) << "CREATING REMOTE BUFFER: " << key;
...
void dsm::Server::fetchRemoteBuffer(BufferName name, uint32_t addr, uint8_t clientID, uint8_t serverID) {
    ...
    LOG(_logger, logger::severityLevel::trace) << "FETCHING REMOTE BUFFER: " << key;
...
void dsm::Server::removeLocalBuffer(dsm::LocalBufferKey key) {
    LOG(_logger, logger::severityLevel::trace) << "REMOVING LOCAL BUFFER " << key;
    interprocess::scoped_lock<interprocess_sharable_mutex> mapLock(*_localBufferMapLock);
...
void dsm::Server::removeRemoteBuffer(dsm::RemoteBufferKey key) {
    LOG(_logger, logger::severityLevel::trace) << "REMOVING REMOTE BUFFER " << key;
...
void dsm::Server::sendRequests() {
    ...
    for (auto const &i : _remoteBuffersToFetch) {
        LOG(_logger, logger::severityLevel::trace) << "SENDING REQUEST " << i;
...
void dsm::Server::sendACKs() {
    ...
    for (auto &i : _remoteServersToACK) {
        ...
        if (multicastPort == 0) {
            LOG(_logger, logger::severityLevel::error) << "BUFFER " << i.first << " IS LOCAL ONLY";
        ...
        for (auto const &j : i.second) {
            LOG(_logger, logger::severityLevel::trace) << "SENDING ACK " << i.first << " TO " << j.address().to_v4().to_string() << " " << j.port();
            _senderSocket.async_send_to(asio::buffer(sendBuffer),
                                        j,
                                        boost::bind(&dsm::Server::sendHandler,
                                                    this,
...
void dsm::Server::sendData() {
    interprocess::sharable_lock<interprocess_sharable_mutex> mapLock(*_localBufferMapLock);
    for (auto const &i : *_localBufferMap) {
        LOG(_logger, logger::severityLevel::periodic) << "SENDING DATA " << i.first << " TO " << std::get<3>(i.second).address() << " " << std::get<3>(i.second).port();
...
void dsm::Server::processRequest(ip::udp::endpoint remoteEndpoint) {
    ...
    LOG(_logger, logger::severityLevel::info) << "RECEIVED REQUEST FOR " << name << " FROM " << remoteEndpoint.address().to_string() << " " << remoteEndpoint.port();
    interprocess::sharable_lock<interprocess_sharable_mutex> mapLock(*_localBufferMapLock);
    if (_localBufferMap->find(name) != _localBufferMap->end()) {
        ...
        LOG(_logger, logger::severityLevel::error) << "COULDN'T FIND BUFFER " << name;
    }
}
...
void dsm::Server::processACK(ip::udp::endpoint remoteEndpoint, bool localOnly) {
    ...
    LOG(_logger, logger::severityLevel::info) << "RECEIVED ACK FOR " << key;
    ...
    _remoteBuffersToFetch.erase(key);
    if (localOnly) {
        LOG(_logger, logger::severityLevel::error) << "BUFFER " << key << " IS MARKED LOCAL ONLY";
        return;
    }
...
void dsm::Server::processData(const boost::system::error_code &error, size_t bytesReceived, dsm::RemoteBufferKey key, boost::shared_ptr<ip::udp::socket> sock, ip::udp::endpoint sender, boost::shared_ptr<asio::deadline_timer> timer) {
    LOG(_logger, logger::severityLevel::periodic) << "RECEIVED DATA FOR REMOTE " << key;
    if (error) {
        LOG(_logger, logger::severityLevel::error) << "ERROR PROCESSING DATA FOR " << key;
    ...
    if (iterator == _remoteBufferMap->end()) {
        LOG(_logger, logger::severityLevel::error) << "MAP ENTRY FOR " << key << " DOESN'T EXIST";
        _remoteReceiveBuffers.erase(key);
        return;
    }
    uint16_t len = std::get<1>(iterator->second);
    if (len != bytesReceived) {
        LOG(_logger, logger::severityLevel::error) << "RECEIVED " << bytesReceived << " BYTES WHEN " << len << " WERE EXPECTED";
        _remoteReceiveBuffers.erase(key);
        return;
    }
    ...
    if (!std::get<3>(iterator->second)) {
        ...
        LOG(_logger, logger::severityLevel::info) << "BUFFER " << key << " IS NOW ACTIVE";
    }
...
void dsm::Server::setBufferToInactive(const boost::system::error_code &error, dsm::RemoteBufferKey key) {
    if (error) {
        if (error != boost::asio::error::operation_aborted) {
            LOG(_logger, logger::severityLevel::error) << "ERROR SETTING BUFFER INACTIVE" << error.message();
        }
        return;
    }
    interprocess::scoped_lock<interprocess_sharable_mutex> mapLock(*_remoteBufferMapLock);
    ...
    LOG(_logger, logger::severityLevel::info) << "BUFFER " << key << " IS NOW INACTIVE";
}

void dsm::Server::senderThreadFunction() {
    LOG(_logger, logger::severityLevel::startup) << "SENDER START";
    while (_isRunning.load()) {
        LOG(_logger, logger::severityLevel::periodic) << "SENDER";
        sendRequests();
        sendACKs();
        sendData();
        boost::this_thread::sleep_for(boost::chrono::milliseconds(SENDER_DELAY));
    }
    LOG(_logger, logger::severityLevel::teardown) << "SENDER END";
}

void dsm::Server::receiverThreadFunction() {
    LOG(_logger, logger::severityLevel::startup) << "RECEIVER START";
    boost::system::error_code err;
    ip::udp::endpoint remoteEndpoint;
    while (_isRunning.load()) {
        _receiverSocket.receive_from(asio::buffer(_receiveBuffer), remoteEndpoint, 0, err);

        LOG(_logger, logger::severityLevel::periodic) << "RECEIVER GOT PACKET";
        ...
        switch (_receiveBuffer[0]) {
            ...
            case 2:
                ...
                break;
            default:
                LOG(_logger, logger::severityLevel::info) << "IGNORING PACKET";
                continue;
        }
    }
    LOG(_logger, logger::severityLevel::teardown) << "RECEIVER END";
}

void dsm::Server::handlerThreadFunction() {
    LOG(_logger, logger::severityLevel::startup) << "HANDLE RECEIVE START";
    _ioService.run();
    LOG(_logger, logger::severityLevel::teardown) << "HANDLE RECEIVE END";
}
